...
def send_spike_sms(counts_per_workflow: dict, total: int, contacts: list, window_end: datetime) -> None:
    flow_parts = "\n".join(
        f"• Flow {i+1}: {v} leads"
    ...
    message = (
        ...
        f"🕐 {_ams_time(window_end)}\n"
        f"\n"
        f"*{total} nieuwe leads in 5 minuten toegevoegd aan de WhatsApp flows*\n"
        f"\n"
        f"*Per workflow:*\n"
        f"{flow_parts}\n"
        f"\n"
